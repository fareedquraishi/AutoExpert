        ...
        viewModelScope.launch {
            session.baId.map { it ?: "" }.filter { it.isNotEmpty() }.collect { baId ->
                ...
                _uiState.update { it.copy(
                    ...
                    reachTarget  = if (target?.basis == "reach"  || target?.basis == "entries") target.targetValue else 100.0,
                    litresTarget = if (target?.basis == "litres" || target?.basis == "volume")  target.targetValue else 100.0,
                )}
            }
        }
    }
            ...
            try {
                ...
                api.getNotices(apiKey = apiKey, auth = authHeader).body()?.let { list ->
                    noticeDao.upsertAll(list.map { n -> NoticeEntity(
                        id = n.id, title = n.message, body = n.message,
