        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (currentStation.isNullOrEmpty()) {
                    ...
                    if (stationId.isNotEmpty()) {
                        api.getStations(apiKey = apiKey, auth = authHeader).body()?.find { it.id == stationId }?.let { station ->
                            ...
                            val lat = session.lat.first()
                            val lng = session.lng.first()
                            val radius = session.radius.first()
                            session.saveSession(
                ...
                api.getSaleEntries(
                    ...
                    entries.forEach { e ->
                        if (e.id != null) {
                            val existing = saleDao.getByRemoteId(e.id)
                            if (existing == null) {
                                saleDao.upsert(SaleEntryQueueEntity(
                                    localId = e.id,
                                    remoteId = e.id,
                                    baId = e.baId,
